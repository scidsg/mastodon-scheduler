...
def index():
    ...
    if request.method == "POST":
        ...
        if image and image.filename != "":
            ...
            if not mimetype:
                error_message = (
                    "Could not determine the MIME type of the uploaded file."
                )
            else:
                try:
                    media = mastodon.media_post(
                        image, mime_type=mimetype, description=alt_text
                    )
                    media_id = media["id"]
                except Exception as e:
        ...
        if scheduled_time and not error_message:
            try:
                ...
                error_message = f"Error scheduling toot: {e}"
    ...
    try:
        scheduled_statuses = []
        # Use make_pagination_iterator to fetch all scheduled statuses
        for status in mastodon.make_pagination_iterator(mastodon.scheduled_statuses):
            scheduled_statuses.append(status)
        ...
        # Parse and sort the scheduled statuses by their scheduled time in ascending order
        for status in scheduled_statuses:
            if isinstance(status["scheduled_at"], str):
                # Convert string to datetime object
                try:
                    status["scheduled_at_parsed"] = dateutil.parser.parse(
                        status["scheduled_at"]
                    )
                except Exception as e:
        ...
        # Process media attachments if needed
        for status in scheduled_statuses:
    ...
    try:
        ...
        if scheduled_statuses:
            ...
            sorted_statuses = sorted(
                scheduled_statuses, key=lambda x: x["scheduled_at"]
            )
            next_post = sorted_statuses[0]  # Get the earliest scheduled post

            post_data = {
                "content": next_post["params"]["text"],
                "image_path": next_post["media_attachments"][0]["url"]
                if next_post["media_attachments"]
                else None,
                "image_alt_text": next_post["media_attachments"][0]["description"]
                if next_post["media_attachments"]
                else None,
